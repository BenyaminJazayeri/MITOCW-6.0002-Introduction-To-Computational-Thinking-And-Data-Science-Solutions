...
class Patient(object):
    ...
    def update(self):
        ...
        #initialize new list
        new_bacteria = []
        #exclude dead bacteria
        for single_bacteria in self.bacteria:
            if not single_bacteria.is_killed():
                new_bacteria+=[single_bacteria]
        #reproduce
        pop_density = len(self.bacteria)/self.max_pop
        for single_bacteria in new_bacteria:
            try:
                new_bacteria+=[single_bacteria.reproduce(pop_density)]
...
def simulation_without_antibiotic(num_bacteria,
    ...
    for trial in range(num_trials):
        ...
        trial_pops=[]
        for i in range(300):
            trial_pops+=[patient.get_total_pop()]
            patient.update()
        populations+=[trial_pops]
    make_one_curve_plot(list(range(300)),[calc_pop_avg(populations, n) for n in range(300)],'elapsed time steps','bacteria population size','Without Antibiotics')
    return populations
...
def calc_pop_std(populations, t):
    ...
    for trial in populations:
        avg_squared_distance += (trial[t]-mean)**2 /len(populations)
    
    return avg_squared_distance**0.5
        ...
        for single_bacteria in self.bacteria:
            if not single_bacteria.is_killed():
                if self.on_antibiotic:
                    if single_bacteria.get_resistant():
                        surviving_bacteria+=[single_bacteria]
                else:
                    surviving_bacteria+=[single_bacteria]
        pop_density = len(surviving_bacteria)/self.max_pop
        for single_bacteria in self.bacteria:
            try:
                surviving_bacteria+=[single_bacteria.reproduce(pop_density)]
            except NoChildException:
                pass
        self.bacteria=surviving_bacteria
        return len(surviving_bacteria)
...
def simulation_with_antibiotic(num_bacteria,
    ...
    for trial in range(num_trials):
        ...
        for i in range(150):
            total_trial+=[patient.get_total_pop()]
            resistant_trial+=[patient.get_resist_pop()]
            patient.update()
        ...
        for i in range(250):
            ...
            resistant_trial+=[patient.get_resist_pop()]
            patient.update()
        total_populations+=[total_trial]
        resistant_populations+=[resistant_trial]
        
    tot = [calc_pop_avg(total_populations, t) for t in range(400)]
    res = [calc_pop_avg(resistant_populations, t) for t in range(400)]
    make_two_curve_plot(range(400), tot, res, 'total bacteria', 'resistant bacteria', 'time', 'population', 'With Antibiotics')
