...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        """
        if width<=0:
            raise ValueError('width not >0')
        self.width = width
        if height<=0:
            raise ValueError('height not >0')
        self.height = height
        if dirt_amount<0:
            raise ValueError('dirt amount not >=0')
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        ...
        while coverage<min_coverage:
            ...
            time_step+=1
        trial_vals+=[time_step]
    return sum(trial_vals)/len(trial_vals)
